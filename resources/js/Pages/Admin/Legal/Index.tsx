...
export default function LegalIndex({ documents }: Props) {
    ...
    const form = useForm({ name: '', number: '', issuer: '', issued_date: '', expiry_date: '' });
    const editForm = useForm({ name: '', number: '', issuer: '', issued_date: '', expiry_date: '', is_active: true });
        ...
        <AdminLayout title="Legalitas">
            ...
            <form onSubmit={handleCreate} className="bg-aru-putih rounded-lg p-6 mb-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-[11px] uppercase tracking-wide text-aru-abu mb-1">Nama Dokumen *</label>
                    ...
                    <div>
                        <label className="block text-[11px] uppercase tracking-wide text-aru-abu mb-1">Penerbit</label>
                        ...
                        {editId === d.id ? (
                            <form onSubmit={handleUpdate} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label className="block text-[11px] uppercase tracking-wide text-aru-abu mb-1">Nama Dokumen</label>
                                        <input type="text" value={editForm.data.name} onChange={e => editForm.setData('name', e.target.value)} className="border rounded px-3 py-2 text-sm w-full" />
                                    </div>
                                    <div>
                                        <label className="block text-[11px] uppercase tracking-wide text-aru-abu mb-1">Nomor</label>
                            ...
                            <div className="flex items-start justify-between">
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="font-semibold text-aru-biru-tua">{d.name}</h3>
                                    </div>
                                    {d.number && <p className="text-sm text-aru-abu mt-1 font-mono">No. {d.number}</p>}
                                    {d.issuer && <p className="text-sm text-aru-abu">Penerbit: {d.issuer}</p>}
                                    {d.issued_date && (
